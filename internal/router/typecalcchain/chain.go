...
// typecalc.CycleCap — exhausting the cycle counter is the same event
...
func BuildChain(d Deps) (*router.Router, error) {
	...
	// Described → synthesize.
	r.Register(&router.HandlerFunc{
		In:  TypeDescribed,
		Out: []string{TypeSynthesized, TypeObstacle},
		Run: func(ctx context.Context, in router.TypedValue) (router.TypedValue, error) {
			...
			if n == 0 {
				// CANNOT_SYNTHESIZE — agent must produce obstacle+waiver
				// pair manually. Emit obstacle and let the upper-level
				// driver pair it with a waiver if appropriate.
...
func makeObstacle(objectID, reason, lastType string) router.TypedValue {
	// v9.0.1 E — persist the chain-emitted obstacle to the bundle's
	// Obstacle section so (a) the agent's next confirm_object call
	// (or session_gate_check) sees the obstacle without re-deriving
	// it from log scrape, and (b) gate.readObstacleReason returns
	// the actual chain-diagnosed text instead of "".
	// Cycles count is best-effort — read existing bundle, fall back
	// to 0 if absent (the cycles counter is bumped by review/test,
	// not by the chain terminal).
	cyclesCount := 0
	if b, ok := typecalc.ReadBundle(objectID); ok && b.Cycles != nil {
		cyclesCount = b.Cycles.Count
	}
	_ = typecalc.WriteObstacle(&typecalc.ObstacleEvidence{
		ObjectID: objectID,
		Reason:   reason,
		Cycles:   cyclesCount,
	})
