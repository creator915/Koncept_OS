...
func TestStart_AtomicCreateActiveFocus(t *testing.T) {
	...
	s, err := Start(dir, "s_x", "", "test task", session.Input{})
	if err != nil {
	...
	cur, _ := persistence.GetFocus(dir)
	if cur != "s_x" {
...
func TestStart_DuplicateFails(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, "K", "sessions")
	if _, err := Start(dir, "s_x", "", "task", session.Input{}); err != nil {
		t.Fatal(err)
	}
	if _, err := Start(dir, "s_x", "", "task", session.Input{}); err == nil {
...
func TestStart_BadParentRollsBack(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, "K", "sessions")
	_, err := Start(dir, "s_child", "s_missing", "task", session.Input{})
	if err == nil {
		t.Errorf("Start with bad parent should error")
	}
	if persistence.ExistsSession(dir, "s_child") {
