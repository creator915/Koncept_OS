...
func Aggregate(dir, id string) (*session.Session, error) {
	...
	visit = func(sid string) error {
		cur, err := persistence.LoadSession(dir, sid)
		if err != nil {
			return err
		}
		...
		for _, child := range cur.Children {
			if !persistence.ExistsSession(dir, child) {
				continue
			}
...
func deriveOutputsFromState(s *session.Session, cwd string) (impls, sigs, attrs, tests []string) {
	...
	for id := range idSet {
		...
		// persistence.Path is .kcpos/typecalc/<id>.json; the rel string is what
