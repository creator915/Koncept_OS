...
func TestNew_StartsPending(t *testing.T) {
	c := checkpoint.New()
	c.RecomputeSummary()
	if c.Summary.FinalVerdict != checkpoint.VerdictPending {
...
func TestAddItem_RejectsBadInput(t *testing.T) {
	...
	cases := []struct {
		...
		{"bad id", func() error { return AddItem(p, "bad", "desc", "", checkpoint.SeverityMust, "") }},
		{"empty desc", func() error { return AddItem(p, "CHK-001", "", "", checkpoint.SeverityMust, "") }},
		{"bad severity", func() error { return AddItem(p, "CHK-001", "desc", "", "weird", "") }},
		{"waiver no reason", func() error { return AddItem(p, "CHK-001", "desc", "", checkpoint.SeverityWaiver, "") }},
	}
	for _, c := range cases {
		if err := c.fn(); err == nil {
...
func TestAddItem_DuplicateRejected(t *testing.T) {
	p := tempCP(t)
	if err := AddItem(p, "CHK-001", "first", "", checkpoint.SeverityMust, ""); err != nil {
		t.Fatal(err)
	}
	if err := AddItem(p, "CHK-001", "second", "", checkpoint.SeverityMust, ""); err == nil {
...
func TestFreeze_BlocksAdd(t *testing.T) {
	p := tempCP(t)
	if err := AddItem(p, "CHK-001", "desc", "", checkpoint.SeverityMust, ""); err != nil {
		t.Fatal(err)
	}
	if err := Freeze(p); err != nil {
		t.Fatal(err)
	}
	if err := AddItem(p, "CHK-002", "later", "", checkpoint.SeverityMust, ""); err == nil {
...
func TestFreeze_Idempotent(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "desc", "", checkpoint.SeverityMust, "")
	if err := Freeze(p); err != nil {
		t.Fatal(err)
	}
	if err := Freeze(p); err != nil {
...
func TestVerdict_PendingBeforeFreeze(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "desc", "", checkpoint.SeverityMust, "")
	c, _ := persistence.LoadCheckpoint(p)
	if c.Summary.FinalVerdict != checkpoint.VerdictPending {
...
func TestVerdict_FailWhenMustUnfilled(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "must check", "", checkpoint.SeverityMust, "")
	AddItem(p, "CHK-002", "should check", "", checkpoint.SeverityShould, "")
	Freeze(p)
	c, _ := persistence.LoadCheckpoint(p)
...
func TestVerdict_PassWhenAllMustsFilled(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "must check", "", checkpoint.SeverityMust, "")
	AddItem(p, "CHK-002", "should check", "", checkpoint.SeverityShould, "") // unfilled, should-only
	Freeze(p)
	if err := Fill(p, "CHK-001", "src/foo.ts:42 Foo"); err != nil {
		t.Fatal(err)
	}
	c, _ := persistence.LoadCheckpoint(p)
	if c.Summary.FinalVerdict != checkpoint.VerdictPass {
...
func TestVerdict_WaiverCountsAsResolved(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "must check", "", checkpoint.SeverityMust, "")
	AddItem(p, "CHK-002", "waived", "", checkpoint.SeverityWaiver, "infeasible to test")
	Freeze(p)
...
func TestFill_RejectsWaiverItem(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "desc", "", checkpoint.SeverityWaiver, "reason")
...
func TestWaive_PostFreezeWorks(t *testing.T) {
	...
	c, _ := persistence.LoadCheckpoint(p)
	if c.Items[0].Severity != checkpoint.SeverityWaiver {
...
func TestPersistence_RoundTrip(t *testing.T) {
	p := tempCP(t)
	AddItem(p, "CHK-001", "desc1", "ui", checkpoint.SeverityMust, "")
	AddItem(p, "CHK-002", "desc2", "core", checkpoint.SeverityShould, "")
	Freeze(p)
	Fill(p, "CHK-001", "src/x.ts:10")

	c, err := persistence.LoadCheckpoint(p)
	if err != nil {
		t.Fatal(err)
	}
	if !c.Frozen {
