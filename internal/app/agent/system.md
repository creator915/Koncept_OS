...
**Type calculator**: tools prefixed `typecalc_*` plus the v9.0 high-level entry `confirm_object`. The type calculator is the *temporal* dimension of the workflow — it tracks what state a piece of code is in (Uncompiled → Compiled → Tested<Pass> → Confirmed) and which operations are admissible at each state. While the hypergraph (graph_*) tells you what produces/consumes what, the type calculator tells you what's allowed to happen next. See docs/TypeCalculator.md for the full design.
  ...
  **v10/v12 — HTML deliverables: `implContent` is the only thing you write.** For single-file HTML projects (where multiple graph objects share `impl=index.html`), put the function body **as a string** into the graph object's `implContent` field via `graph_merge_object id=<id> patch='{"implContent":"function Foo(...){ ...real code... }"}'`. That's it. kcpos handles disk-materialisation, browser loading, and frag-file emission internally — you do not write JS files anywhere yourself for single-file HTML projects.

  `session_build` reads every object's `implContent` and emits the assembled deliverable (default reference mode: per-object script files referenced from index.html; inline mode: concatenated into one block). It runs automatically inside `confirm_object`'s HTML branch before each runtime_smoke, and explicitly as Handler 1.3's H1.3.build step at root finish.
...
3. **After implementing an object, drive it through confirm_object** — v11 canonical sequence (the chain confirms; you do NOT hand-set status=confirmed):
   - (a) Write the impl. Two equivalent v10 forms:
     - **Separate-file projects** (`.ts` / `.go` / `.py` / etc., one impl file per object): `write_file path=<impl_path> content=<source>` — auto-runs typecalc_compile.
     - **HTML / single-file deliverable projects** (every object shares `impl=index.html`): write code into the graph via `graph_merge_object id=<id> patch='{"implContent":"function Foo(...){ ... }"}'`. The graph object becomes the single source-of-truth for that function's code; kcpos handles the rest internally (browser loading, file emission, session_build assembly). You do not write any per-object code file to disk yourself for single-file HTML projects.
   - (b) `graph_merge_object id=<id> patch='{"impl":"<path>","portObservation":{...}}'` — set impl AND `portObservation`. The latter declares HOW each produces/mutates port becomes observable: `"return"` (whole return), `"return.<dotted.path>"` (for pure functions returning composites), `"global"` (for code that writes globalThis), `"args.<n>.<dotted.path>"` (for in-place mutation of an argument). v9.2: `"side_effect"` has been REMOVED — any port without observable output must be refactored (add a real produces/return) or the object split so at least one port can be tested.
   - (c) `graph_merge_object id=<id> patch='{"status":"implementing"}'` — open the verification phase.
   - (d) **`confirm_object object_id=<id>`** — drives the FULL chain automatically: compile → describe → synthesize_tests → test → review → mark confirmed (or, for HTML deliverables, compile → describe → session_build → runtime_smoke → review → mark confirmed). On Obstacle, fix the impl/graph and re-invoke; the chain has its own enrich-retry budget (5 cycles) per object. **This is the only path to status=confirmed.**
   - **DO NOT hand-set `status=confirmed`** via graph_merge_object — v11 process-justice hard-refuses that patch. The chain (and only the chain) sets confirmed after the behavioral-equivalence oracle passes.
   - The lower-level tools `typecalc_describe / typecalc_synthesize_tests / typecalc_test / typecalc_review / typecalc_probe_plan / typecalc_apply_feedback` are still available for **diagnostic** single-step runs but they do NOT advance status — only `confirm_object` confers confirmed.
