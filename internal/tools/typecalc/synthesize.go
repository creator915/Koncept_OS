...
func typecalcSynthesizeTestsTool() llm.Tool {
	return llm.Tool{
		...
		Run: func(ctx context.Context, args map[string]interface{}) (string, error) {
			...
			spec, ok := typecalc.ReadSpec(objectID)
			if !ok {
			...
			if existing, ok := typecalc.ReadTests(objectID); ok && existing.SpecHash == spec.SourceHash && len(existing.TestCode) > 0 {
				return fmt.Sprintf(
					"synthesized tests for %s [cache hit, no LLM call] — kept %s\n\n--- test code (cached) ---\n%s",
					objectID,
					typecalc.TestsEvidencePath(objectID),
					existing.TestCode,
				), nil
			}
			...
			if lang == "" && obj.Impl != nil && *obj.Impl != "" {
				ext := extOf(*obj.Impl)
				lang = string(typecalc.LangFromExt(ext))
			}
			...
			out, err := typecalc.SynthesizeTests(ctx, typecalc.SynthesizeInputs{
				ObjectID:        objectID,
				Lang:            lang,
				Intent:          obj.Intent,
			...
			if err := typecalc.WriteTests(rec); err != nil {
