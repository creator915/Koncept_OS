...
//  4. Run typecalc.CompileLanguageInvoker. On Compiled, write evidence
...
func autoCompileAfterWrite(ctx context.Context, path, content string) string {
	objectIDs, lang := resolveAutoCompileTarget(path)
	if len(objectIDs) == 0 {
		return ""
	}
	...
	if out.State == typecalc.StateCompiled {
		// Fix 1: HTML containing inline <script> is effectively a JS
		// container — record evidence under JavaScript so the gate
		// requires test evidence (you can't dodge testing by writing
		// JS inside HTML).
		effectiveLang := detectEffectiveLang(content, lang)
		var written []string
		for _, id := range objectIDs {
			if recErr := recordTypecalcEvidence(id, "compile", string(effectiveLang), true); recErr == nil {
