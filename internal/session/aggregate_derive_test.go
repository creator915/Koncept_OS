...
func TestAggregate_DerivesImplFromAddedAndModifiedObjects(t *testing.T) {
	...
	s.Output.GraphDiff.Modified.Objects["Modified"] = ModifiedRecord{
		Before: beforeRaw,
		After:  afterRaw,
	}
	if err := Save(sessionDir, s); err != nil {
		t.Fatal(err)
	}
...
func TestAggregate_WalksDescendantsAndDerives(t *testing.T) {
	...
	if _, err := Create(sessionDir, "s_a", "s_root", "child a", Input{}); err != nil {
		t.Fatal(err)
	}
	if _, err := Create(sessionDir, "s_b", "s_root", "child b", Input{}); err != nil {
		t.Fatal(err)
	}
