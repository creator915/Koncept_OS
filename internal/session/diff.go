...
// CaptureDiff compares before and after, computes the structural diff, and
// merges it into the focused session's graphDiff. No-op if no session is
// focused, focus points to a missing session, or the focused session is not
// in active state.
//
// Merge semantics across multiple captures within the same session:
//   - added: a new id present in `after` but not `before` is recorded; if
//     a later capture shows the same id was modified, the snapshot in
//     `added` is updated to the latest `after`.
//   - modified: tracked once; on subsequent captures the original `before`
//     is preserved and `after` is replaced with the newest state.
//   - removed: ids present in `before` but not `after` go to `removed`. If
//     the same id was tracked as added in this session, it is removed from
//     `added` (net: nothing happened from the session's perspective).
func CaptureDiff(sessionDir string, before, after *graph.Graph) error {
	focusID, err := GetFocus(sessionDir)
	if err != nil || focusID == "" {
		return nil
	}
	if !Exists(sessionDir, focusID) {
		// stale focus pointer; clear it so future calls don't keep retrying
		_ = SetFocus(sessionDir, "")
		return nil
	}
	s, err := Load(sessionDir, focusID)
	if err != nil {
		return nil
	}
