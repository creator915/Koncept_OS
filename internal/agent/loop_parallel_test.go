...
func TestRunBatchConcurrent_ActuallyParallel(t *testing.T) {
	...
	tool := llm.Tool{
		...
		Spec: llm.ToolSpec{
			Type:     "function",
			Function: llm.ToolFunction{Name: "slowtool"},
		},
	...
	builtins := map[string]llm.Tool{"slowtool": tool}
	batch := []llm.ToolCall{
...
func TestRunBatchConcurrent_PreservesResultOrder(t *testing.T) {
	...
	tool := llm.Tool{
		Concurrent: true,
		Spec:       llm.ToolSpec{Type: "function", Function: llm.ToolFunction{Name: "ordered"}},
		Run: func(ctx context.Context, args map[string]interface{}) (string, error) {
	...
	builtins := map[string]llm.Tool{"ordered": tool}
	batch := []llm.ToolCall{
...
func TestIsConcurrent_DefaultsFalseForUnknown(t *testing.T) {
	builtins := map[string]llm.Tool{
		"yes": {Concurrent: true, Spec: llm.ToolSpec{Function: llm.ToolFunction{Name: "yes"}}, Run: nil},
		"no":  {Concurrent: false, Spec: llm.ToolSpec{Function: llm.ToolFunction{Name: "no"}}, Run: nil},
	}
...
func mkCall(name, args, id string) llm.ToolCall {
	return llm.ToolCall{
		ID:   id,
		Type: "function",
