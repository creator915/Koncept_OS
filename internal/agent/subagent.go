...
func NewSubAgentRunnerWithCaps(client *llm.Client, depth int, parentCaps typecalc.CapSet) *SubAgentRunner {
...
//   - Role non-empty: resolve via typecalc.PresetByName.
...
func (r *SubAgentRunner) resolveChildCaps(req tools.SubAgentRequest) (typecalc.CapSet, error) {
	var child typecalc.CapSet
	switch {
	case req.Role != "":
		preset, ok := typecalc.PresetByName(req.Role)
		if !ok {
			return nil, fmt.Errorf("unknown role %q (allowed: implementer / tester / integrator / root)", req.Role)
		}
		child = preset
	case len(req.Caps) > 0:
		child = make(typecalc.CapSet, len(req.Caps))
		for i, t := range req.Caps {
			child[i] = typecalc.Capability(t)
		}
	default:
