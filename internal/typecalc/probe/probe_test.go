...
func TestPlanFromGraph_EmptyGraph(t *testing.T) {
	plan, err := PlanFromGraph(graph.NewGraph())
	if err != nil {
		t.Fatal(err)
	}
...
func TestLocateFaultFromObservations_Heuristic(t *testing.T) {
	...
	if out.Kind != "FaultLocated" {
		t.Fatalf("got %v", out.Kind)
	}
	// Smoke encode/decode round-trip on the plan.
	if _, err := DecodePlan(NewPlan(plan)); err != nil {
		t.Fatalf("DecodePlan round-trip: %v", err)
	}
}
