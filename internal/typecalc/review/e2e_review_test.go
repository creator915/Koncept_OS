...
func TestEndToEnd_DescribeReviewAccept(t *testing.T) {
	...
	if err := core.RecordEvidence("Process", "test", "Go", true); err != nil {
		t.Fatal(err)
	}
	...
	back, ok := core.ReadAccepted("Process")
	if !ok {
	...
	if back.SourceHash != core.HashSource(implContent) {
	...
	expectedPath := core.AcceptedEvidencePath("Process")
...
func TestEndToEnd_StaleSpecBlocksReview(t *testing.T) {
	...
	if err := core.WriteSpec(&core.SpecEvidence{
		ObjectID:    "Process",
		Description: "noop",
		SourceHash:  core.HashSource(originalImpl),
	}); err != nil {
		t.Fatal(err)
	}
...
func stubInvoker(reply string) func(ctx context.Context, prompt string) (string, error) {
