...
func TestDescribeWithInvoker_RejectsEmpty(t *testing.T) {
	_, err := synthesize.DescribeWithInvoker(context.Background(),
		synthesize.DescribeInputs{ObjectID: "F"},
		func(ctx context.Context, prompt string) (string, error) {
			return "   ", nil
		})
	if err == nil {
...
func TestEvidenceRoundTrip_SpecAndAccepted(t *testing.T) {
	...
	spec := &core.SpecEvidence{
		ObjectID:    "Foo",
		Description: "does foo things",
		SourceHash:  core.HashSource("source content"),
	}
	if err := core.WriteSpec(spec); err != nil {
		t.Fatalf("write spec: %v", err)
	}
	back, ok := core.ReadSpec("Foo")
	if !ok {
	...
	accepted := &core.AcceptedEvidence{
		ObjectID: "Foo",
		OK:       false,
		StaticIssues: []core.StaticIssue{
			{Code: "test-code", Where: "Foo", Message: "x"},
		},
		Reasonableness: core.ReviewVerdict{
			Verdict: "fail", Reasons: []string{"y"}, Confidence: 0.3,
		},
	}
	if err := core.WriteAccepted(accepted); err != nil {
		t.Fatalf("write accepted: %v", err)
	}
	back2, ok := core.ReadAccepted("Foo")
	if !ok {
