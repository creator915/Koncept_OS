...
// provider.ProviderFromEnv so the same provider/key the agent uses is reused.
// In test environments without an API key, a stub reviewer
// (see ReviewWithInvoker) lets callers inject deterministic verdicts.
func ReviewReasonableness(ctx context.Context, in ReviewInputs) (core.ReviewVerdict, error) {
	cfg, err := provider.ProviderFromEnv()
	if err != nil {
		return core.ReviewVerdict{}, fmt.Errorf("review: load llm config: %w", err)
	}
...
func ReviewWithInvoker(ctx context.Context, in ReviewInputs, invoke Invoker) (core.ReviewVerdict, error) {
	prompt := buildReviewPrompt(in)
	reply, err := invoke(ctx, prompt)
	if err != nil {
		return core.ReviewVerdict{}, fmt.Errorf("review: llm call: %w", err)
	}
	...
	if perr != nil {
		...
		return core.ReviewVerdict{}, fmt.Errorf("review: parse reply: %w (raw: %s)", perr, truncate(reply, 500))
	}
	if verdict.Verdict != "pass" && verdict.Verdict != "fail" {
		return core.ReviewVerdict{}, fmt.Errorf("review: invalid verdict %q (must be pass | fail)", verdict.Verdict)
	}
	return verdict, nil
}
...
func parseReviewReply(reply string) (core.ReviewVerdict, error) {
	...
	if match == "" {
		return core.ReviewVerdict{}, fmt.Errorf("no JSON object found in reply")
	}
	var v core.ReviewVerdict
	if err := json.Unmarshal([]byte(match), &v); err != nil {
		return core.ReviewVerdict{}, err
	}
	return v, nil
}
