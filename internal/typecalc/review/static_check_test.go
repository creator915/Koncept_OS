...
// useTempEvidenceDir reroutes core.EvidenceDir into a temp directory for the
...
func useTempEvidenceDir(t *testing.T) func() {
	t.Helper()
	prev := core.EvidenceDir
	dir, err := os.MkdirTemp("", "kcpos-typecalc-")
	if err != nil {
		t.Fatalf("mkdir temp: %v", err)
	}
	// core.EvidenceDir is a const — we can't reassign it. Instead, chdir into
...
func TestStaticCheck_FlagsEmptyEffects(t *testing.T) {
	...
	issues := StaticCheck(".", g, "Process").Issues()
...
func TestStaticCheck_DetectsStaleSpec(t *testing.T) {
	...
	if err := core.WriteSpec(&core.SpecEvidence{
		...
		SourceHash:  core.HashSource("package main\n// version 1"),
	}); err != nil {
		t.Fatal(err)
	}
...
func TestStaticCheck_PassesWhenAllOK(t *testing.T) {
	...
	if err := core.RecordEvidence("Process", "compile", "Go", true); err != nil {
		t.Fatal(err)
	}
	...
	if err := core.WriteSpec(&core.SpecEvidence{
		...
		SourceHash:  core.HashSource(string(implContent)),
	}); err != nil {
		t.Fatal(err)
	}
...
func hasIssue(issues []core.StaticIssue, code string) bool {
...
func TestStaticIssue_StringNonEmpty(t *testing.T) {
	is := core.StaticIssue{Code: "x", Where: "y", Message: "z"}
...
func TestBugDFixture_OrphanKeyMessageReachesAgent(t *testing.T) {
	...
	// v9.3.1: bug-D fixture uses a JS impl (not .html) — under v9.3 HTML
	// deliverables are verified via runtime_smoke and don't use the
	// portObservation harness mechanism at all, so port-observation-required
	// is deliberately skipped for HTML. The bug-D diagnostic (orphan +
	// required combined signal) still matters for non-HTML test-harness
	// flows. Pong-01's original scenario was modelled with HTML impl
	// historically; v9.3 has obsoleted that exact form, so the regression
	// test moves to JS where the combined signal remains live.
	implPath := "src/InitGame.impl.js"
	...
	if err := os.WriteFile(implPath, []byte("function InitGame(){return{ball:{},paddle:{},gameStatus:'playing'}}"), 0o644); err != nil {
		t.Fatal(err)
	}

	issues := StaticCheck(".", g, "InitGame").Issues()

	// Must fire orphan-key for the camelCase key.
	var orphan *core.StaticIssue
	var missing []string
	for i := range issues {
...
function Bar() { return 2; }
	...
	fooFragV1, _ := core.SymbolFragmentHash(html, implPath, "Foo")
	barFragV1, _ := core.SymbolFragmentHash(html, implPath, "Bar")
	...
	fileHashV1 := core.HashSource(html)
	...
	for _, o := range []struct{ id, frag string }{{"Foo", fooFragV1}, {"Bar", barFragV1}} {
		...
		if err := core.RecordEvidenceWithSymbol(o.id, "compile", "JavaScript", true, "", fileHashV1, o.frag); err != nil {
	...
	for _, id := range []string{"Foo", "Bar"} {
		issues := StaticCheck(".", g, id).Issues()
		if hasIssue(issues, "spec-stale") {
...
function Bar() { return 2; }
	...
	fooFragV2, _ := core.SymbolFragmentHash(htmlV2, implPath, "Foo")
	barFragV2, _ := core.SymbolFragmentHash(htmlV2, implPath, "Bar")
...
func TestStaticCheck_RelativeImplResolvesAgainstCwd(t *testing.T) {
	...
	wd, _ := os.Getwd()
	issues := StaticCheck(wd, g, "Process").Issues()
