...
func runSessionCreate(args []string) int {
	...
	s, err := session.Create(session.DefaultDir, normID, *parent, *task, session.Input{})
	if err != nil {
		return printErr(err)
	}
...
func runSessionStatus(args []string) int {
	...
	s, err := session.SetStatus(session.DefaultDir, normID, session.Status(*to))
	if err != nil {
		return printErr(err)
	}
...
func runSessionFocus(args []string) int {
	...
	if *id == "" {
		// Show current focus
		cur, err := session.GetFocus(session.DefaultDir)
		if err != nil {
			return printErr(err)
		}
		if cur == "" {
	...
	s, err := session.Load(session.DefaultDir, normID)
	if err != nil {
		return printErr(err)
	}
	...
	if err := session.SetFocus(session.DefaultDir, normID); err != nil {
		return printErr(err)
	}
	fmt.Printf("focus → %s\n", normID)
	return 0
...
func runSessionDelete(args []string) int {
	...
	deleted, err := session.Rollback(session.DefaultDir, graphPath, normID)
	if err != nil {
		return printErr(err)
	}
	if len(deleted) == 0 {
...
func runSessionResume(args []string) int {
	...
	case session.StatusWaiting:
		...
		if _, err := session.SetStatus(session.DefaultDir, id, session.StatusActive); err != nil {
			return printErr(err)
		}
